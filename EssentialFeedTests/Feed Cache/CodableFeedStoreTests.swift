...
//  EssentialFeedTests
//
//  Created by Abdoulaye Diallo on 6/22/21.
//
        ...
        sut.insert(feed, timestamp: timestamp){ insertionError in
            ...
            sut.retrieve { retrieveResult in
                switch retrieveResult {
                    ...
                    default:
                        XCTFail("Expected found resultwith feed \(feed) and timestamp \(timestamp), got \(retrieveResult) instead")
                }
                exp.fulfill()
            }
        }
    ...
    func test_hasNoSideEffectsOnNonEmptyCache(){
        ...
        sut.insert(feed, timestamp: timestamp){ insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    switch (firstResult, secondResult) {
                        case let (.found(feed: firstFound), .found(secondFound)):
